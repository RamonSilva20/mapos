<?php
/**
 * Script para adicionar campo detalhes na tabela servicos_os
 * Execute este arquivo uma vez através do navegador
 * 
 * URL: https://mapos.tecnicolitoral.com/add_campo_detalhes_servicos_os.php
 */
...
echo "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Adicionar Campo Detalhes</title>";
...
echo "<h2>Adicionando campo detalhes na tabela servicos_os</h2>";
...
try {
    ...
    $check_query = "SHOW COLUMNS FROM `servicos_os` LIKE 'detalhes'";
    $result = $conn->query($check_query);
    
    if ($result && $result->num_rows == 0) {
        // Adicionar campo
        $sql = "ALTER TABLE `servicos_os` ADD COLUMN `detalhes` TEXT NULL AFTER `preco`";
        
        if ($conn->query($sql)) {
            echo "<div class='success'>✓ Campo 'detalhes' adicionado com sucesso na tabela servicos_os!</div>";
        } else {
            echo "<div class='error'>✗ Erro ao adicionar campo: " . htmlspecialchars($conn->error) . "</div>";
        }
    } else {
        echo "<div class='info'>ℹ Campo 'detalhes' já existe na tabela servicos_os.</div>";
    }
