                                ...
                                <form action="<?php echo current_url(); ?>" method="post" id="formVendas">
                                    ...
                                    <div class="span6" style="padding: 1%; margin-left: 0">
                                        ...
                                        <textarea class="editor" name="observacoes_cliente" id="observacoes_cliente"
                                            cols="30" rows="5"><?php echo $result->observacoes_cliente ?></textarea>
                                    </div>
                                        ...
                                        <div class="span1">
                                            ...
                                            <select style="width: 4em;" name="tipoDesconto" id="tipoDesconto">
                                                ...
                                                <option value="porcento"
                                                    <?=$result->tipo_desconto == "porcento" ? "selected" : "" ?>>%
                                                </option>
                                            </select>
                                            <strong><span style="color: red" id="errorAlert"></span></strong>
                                        </div>
    ...
    echo '</tr>';
                                    ...
                                    <tfoot>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div align="center">
                                                    <strong><?php echo $result->tipo_desconto == "real" ? "R$ " : ""; ?>
                                                        <?php echo number_format($result->desconto, 2, '.', ''); ?>
                                                        <?php echo $result->tipo_desconto == "porcento" ? " %" : ""; ?></strong>
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
...
$('#resultado').focusout(function() {
    if (Number($('#resultado').val()) > Number($("#total-venda").val())) {
        $('#errorAlert').text('Desconto não pode ser maior que o Valor.').css("display", "inline").fadeOut(
        6000);
        $('#resultado').val('');
    }
...
$(document).ready(function() {
    ...
    $(document).on('click', '#btn-faturar', function(event) {
        ...
        valor_desconto != 0.00 || valor_desconto ? $('#valor').attr('readonly', false) : $(
            '#faturar-desconto').attr('readonly', false);
        valor = valor.replace(',', '');
        $('#valor').val(valor);
    });
                    ...
                    success: function(data) {
                        if (data.result == true) {
                            ...
                            Swal.fire({
                                type: "error",
                                title: "Atenção",
                                html: "Ocorreu um erro ao tentar adicionar produto. <br /><br />Error: " +
                                    data.messages
                            });
                            $("#divProdutos").load(
                                "<?php echo current_url(); ?> #divProdutos");
    ...
    $(document).on('click', 'a', function(event) {
        ...
        if ((idProduto % 1) == 0) {
            ...
            $.ajax({
                ...
                data: "idProduto=" + idProduto + "&idVendas=" + <?= $result->idVendas ?> +
                ...
                success: function(data) {
                    if (data.result == true) {
                        ...
                        Swal.fire({
                            type: "error",
                            title: "Atenção",
                            html: "Ocorreu um erro ao tentar excluir produto." +
                                data.messages
                        });
                        $("#divProdutos").load("<?php echo current_url(); ?> #divProdutos");
                    }
