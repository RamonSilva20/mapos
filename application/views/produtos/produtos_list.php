	...
	<div class="col-lg-12 col-md-12 col-sm-12">
		<div class="card">
			<div class="card-body">
                ...
                <?= anchor(site_url('produtos/create'),'<i class="fa fa-plus"></i> '.$this->lang->line('app_create'), 'class="btn btn-success"'); ?>    
                      ...
                      <thead>  
                          <tr>  
                            ...
                            <th>#</th>
		                        ...
		                        <th><?= ucfirst($this->lang->line('product_unity')) ?> </th>
		                        <th><?= ucfirst($this->lang->line('product_buy_price')) ?> </th>
		                        <th><?= ucfirst($this->lang->line('product_sell_price')) ?> </th>
		                        <th><?= ucfirst($this->lang->line('product_stock')) ?> </th>
		                        <th><?= ucfirst($this->lang->line('product_min_stock')) ?> </th>
...
<script src="<?= base_url('assets/js/lib/datatables/datatables.min.js'); ?>"></script>
 ...
 $(document).ready(function(){  
      ...
      var datatable = $('#table').DataTable({  
           ...
           "order":[],  
           "ajax":{  
                url:"<?= site_url('produtos/datatable'); ?>",  
                type:"POST"  
           },  
           "columnDefs":[  
                {  
      ...
      $('#form_delete').submit(function(event){
        ...
        $.ajax({
          url: '<?= site_url('produtos/delete_many'); ?>',
          type: 'POST',
          dataType: 'json',
          data: data,
        })
        .done(function(response) {
          ...
          if(response.result == true){
            ...
            toastr.success(response.message,'<?= $this->lang->line('app_attention'); ?>',{
              timeOut: 8000,
              "closeButton": true,
              "newestOnTop": true,
              "progressBar": true,
            ...
            toastr.error(response.message,'<?= $this->lang->line('app_attention'); ?>',{
              timeOut: 8000,
              "closeButton": true,
              "newestOnTop": true,
              "progressBar": true,
