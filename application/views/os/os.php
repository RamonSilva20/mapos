                    ...
                    <?php
                        ...
                        foreach ($results as $r) {
                            ...
                            if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vOs')) {
                                ...
                                $troca = [$r->nomeCliente, $r->idOs, $r->status, 'R$ '.number_format($r->totalProdutos + $r->totalServicos, 2, ',', '.'), strip_tags($r->descricaoProduto),($emitente ? $emitente[0]->nome : ''),($emitente ? $emitente[0]->telefone : ''),strip_tags($r->observacoes),strip_tags($r->defeito),strip_tags($r->laudoTecnico),date('d/m/Y', strtotime($r->dataFinal)),date('d/m/Y', strtotime($r->dataInicial)),$vencGarantia];
