...
class Assinatura extends CI_Controller
{
    public function upload_signature()
    {
        ...
        if($this->input->post('inserirAssCli') && !empty($this->input->post('assClienteImg'))) {
            $data = $this->assCliente($assClienteImg64, $data);
        }

        if($this->input->post('inserirAssTec')) {
            $data = $this->assTecnico($assTecnicoImg64, $data);
        }
    ...
    protected function assCliente($assClienteImg64, $data)
    {
    ...
    protected function assTecnico($assTecnicoImg64, $data)
    {
        $assinaturaImg = $this->usuarios_model->getById($this->session->userdata('id_admin'))->assinaturaImg;
