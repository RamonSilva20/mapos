...
class Financeiro extends MY_Controller
{
    ...
    public function lancamentos()
    {
        ...
        $this->data["configuration"]["base_url"] = site_url(
            "financeiro/lancamentos/?vencimento_de=$vencimento_de&vencimento_ate=$vencimento_ate&cliente=$cliente&tipo=$tipo&status=$status&periodo=$periodo"
        );
        $this->data["configuration"][
            "total_rows"
        ] = $this->financeiro_model->count("lancamentos", $where);
        ...
        $this->data[
            "estatisticas_financeiro"
        ] = $this->financeiro_model->getEstatisticasFinanceiro2();
    ...
    public function adicionarReceita()
    {
        if (
            ...
            $this->session->set_flashdata(
                "error",
                "Você não tem permissão para adicionar lançamentos."
            );
            redirect(base_url());
        }
        ...
        if ($this->form_validation->run("receita") == false) {
            $this->data["custom_error"] = validation_errors()
                ? '<div class="form_error">' . validation_errors() . "</div>"
                : false;
        } else {
            ...
            $valor_desconto = floatval(
                str_replace(",", ".", $this->input->post("valor_desconto"))
            );
            ...
            $data = [
                ...
                "data_pagamento" =>
                    $recebimento != null ? $recebimento : date("Y-m-d"),
            ...
            if ($this->financeiro_model->add("lancamentos", $data) == true) {
                ...
                $this->data["custom_error"] =
                    '<div class="form_error"><p>Ocorreu um erro.</p></div>';
            }
        }
        $this->session->set_flashdata(
            "error",
            "Ocorreu um erro ao tentar adicionar o lançamento."
        );
        redirect($urlAtual);
    }
    ...
    public function adicionarReceita_parc()
    {
        ...
        if (
            ...
            $desconto_por_parcela =
                $valor_desconto > 0 ? $valor_desconto / $qtdparcelas_parc : 0;
            ...
            if ($entrada >= $valor_parc) {
                $this->session->set_flashdata(
                    "error",
                    "O valor da entrada não pode ser maior ou igual ao valor total da receita/Despesa!"
                );
                redirect($urlAtual);
            }
            ...
            try {
                ...
                $dia_pgto =
                    $dia_pgto[2] . "-" . $dia_pgto[1] . "-" . $dia_pgto[0];
            ...
            if ($entrada == 0) {
                ...
                while ($loops <= $qtdparcelas_parc) {
                    ...
                    $data = [
                        "descricao" =>
                            $this->input->post("descricao_parc") .
                            ' - Parcelamento de R$' .
                            $descricao_parc_valor .
                            "  [" .
                            $loops .
                            "/" .
                            $qtdparcelas_parc .
                        ...
                        "data_pagamento" =>
                            $recebimento ?: date_format($myDateTime, "Y-m-d"),
                        "baixado" => 0,
                        "cliente_fornecedor" => $this->input->post(
                            "cliente_parc"
                        ),
                ...
                $data1 = [
                    ...
                    "data_pagamento" =>
                        $dia_pgto != null ? $dia_pgto : date_format("Y-m-d"),
                    "baixado" => 1,
    ...
    public function adicionarDespesa()
    {
        ...
        if ($this->form_validation->run("despesa") == false) {
            ...
            if ($pagamento != null) {
                ...
                $pagamento =
                    $pagamento[2] . "-" . $pagamento[1] . "-" . $pagamento[0];
            }
            ...
            $data = [
                ...
                "data_vencimento" => $vencimento,
                "data_pagamento" =>
                    $pagamento != null ? $pagamento : date("Y-m-d"),
            ...
            if ($this->financeiro_model->add("lancamentos", $data) == true) {
                ...
                $this->session->set_flashdata(
                    "error",
                    "Ocorreu um erro ao tentar adicionar despesa!"
                );
                redirect($urlAtual);
            }
        }
    ...
    public function editar()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (
                ...
                log_info(
                    "Alterou um lançamento no financeiro. ID" .
                        $this->input->post("id")
                );
                redirect($urlAtual);
            } else {
                $this->session->set_flashdata(
                    "error",
                    "Ocorreu um erro ao tentar editar lançamento!"
                );
                redirect($urlAtual);
            }
        }
    ...
    public function excluirLancamento()
    {
        ...
        if ($id == null || !is_numeric($id)) {
            ...
            $result = $this->financeiro_model->delete(
                "lancamentos",
                "idLancamentos",
                $id
            );
    ...
    public function imprimirRecibo()
    {
        if (empty($this->uri->segment(3))) {
            ...
            $data = [
                ...
                "lancamento" => $this->financeiro_model->getLancamento(
                    $this->uri->segment(3)
                ),
                "cliente" => ($data[
                    "cliente"
                ] = $this->financeiro_model->getById(
                    $data["result"]["clientes_id"]
                )),
                "valorporescrito" => $this->numberToText(
                    $this->financeiro_model->getLancamento(
                        $this->uri->segment(3)
                    )["valor"]
                ),
            ];
