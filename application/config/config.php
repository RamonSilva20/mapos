...
// Isso permite acesso via localhost, IP ou domínio
// Detectar protocolo corretamente (HTTP ou HTTPS)
$protocol = 'http://';
if (
...
$host = $_SERVER['HTTP_HOST'] ?? 'localhost:9980';
$port = isset($_SERVER['SERVER_PORT']) && $_SERVER['SERVER_PORT'] != 80 && $_SERVER['SERVER_PORT'] != 443 
    ? ':' . $_SERVER['SERVER_PORT'] 
    : '';
// Remover porta duplicada se já estiver no host
if (strpos($host, ':') !== false) {
    $port = '';
}
// Se for HTTPS na porta padrão, não adicionar porta
if ($protocol === 'https://' && $port === ':443') {
    $port = '';
}
$auto_detected_url = $protocol . $host . $port . '/';

// Usar URL do .env apenas se estiver definida e não for localhost
if (isset($_ENV['APP_BASEURL']) && !empty($_ENV['APP_BASEURL']) && strpos($_ENV['APP_BASEURL'], 'localhost') === false) {
    $config['base_url'] = $_ENV['APP_BASEURL'];
} else {
    // Usar detecção automática
    $config['base_url'] = $auto_detected_url;
}
...
$config['csrf_exclude_uris'] = ['api.*+', 'os/cadastrarClienteRapido', 'mapos/uploadCertificado', 'notasentrada/consultarFila'];
